...
const MovieDetails = () => {
  ...
  const handleFavorite = async () => {
    try {
      if(!user) return TableRowsSplit.error("Please login to proceed");

      const {data} = await axios.post('/api/user/update-favorite', {movie: id}, {headers: {Authorization: `Bearer ${await getToken()}`}})
    ...
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
      <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <h1 className='text-4xl font-semibond max-w-96 text-balance'>{show.movie.title}</h1>
